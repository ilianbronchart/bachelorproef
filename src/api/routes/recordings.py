...
from src.api.models.pydantic_context import RecordingsContext, Request
...
@router.get("/local")
async def local_recordings(request: Request, db: Session = Depends(get_db) ) -> HTMLResponse:
...
async def delete_local_recording(
    ...
    recordings_service.delete(db, recording_id)
    recordings = recordings_service.get_all(db)
    context = RecordingsContext(_request=request, recordings=recordings)
    return templates.TemplateResponse(Template.LOCAL_RECORDINGS, context.model_dump())
...
async def glasses_recordings(request: Request) -> HTMLResponse:
    ...
    if is_hx_request(request):
        return templates.TemplateResponse(Template.GLASSES_RECORDINGS, context.model_dump())
    return templates.TemplateResponse(Template.INDEX, context.model_dump())


@router.get("/glasses/{recording_uuid}/download", response_class=HTMLResponse)
async def download_recording(
    request: Request, recording_uuid: str, db: Session = Depends(get_db)
) -> HTMLResponse:
    ...
    await glasses_service.download_recording(db, recording_uuid)
