...
class SimRoomClass(Base):
    ...
    color: Mapped[str] = mapped_column(String, default="#198754")  # Default color is green

    sim_room: Mapped["SimRoom"] = relationship("SimRoom", back_populates="classes")
    # Each class can be used in many annotations
    annotations: Mapped[List["Annotation"]] = relationship("Annotation", back_populates="sim_room_class")
...
class CalibrationRecording(Base):
    ...
    def get_all() -> List["CalibrationRecording"]:
        with Session(engine) as session:
            return session.query(CalibrationRecording).options(
                joinedload(CalibrationRecording.recording)
            ).all()

    @staticmethod
    def get(id: Union[int, str]) -> Union["CalibrationRecording", None]:
        with Session(engine) as session:
            return (
...
class Annotation(Base):
    ...
    calibration_recording: Mapped["CalibrationRecording"] = relationship("CalibrationRecording", back_populates="annotations")
